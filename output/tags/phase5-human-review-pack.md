...
## ⚡ Phase 5b — עדכון כלל CAT-B (גיל)

**כלל מחייב חדש (אישור אייל):**  
CAT-B (age) נדרש **רק** עבור ביגוד (clothing) ונעליים (shoes).

**לא נדרש age עבור:**
- צעצועים (`type-toy`, `type-plush-toy`)
- בובות ריבורן (`type-reborn-doll`)
- אביזרי אמבטיה (`type-bath-accessory`)
- מוצרי עזר / אלקטרוניקה (`type-accessory`, `type-unknown`)
- מצופי שחייה (`type-swimming-ring`)
...
### מוצר 10 — 🔶 NEEDS_REVIEW — ⚠️ type-unknown — מדחום (taxonomy gap בלבד, לא כשל גיל)
...
**missing / review reason:** ⚠️ **זה מדחום מים — לא בגד, לא נעל, לא בובה.** המערכת נתנה type-unknown בצדק. ⚡ **Phase 5b:** type-unknown → אין חובת גיל. **הבעיה היחידה:** taxonomy gap — `type-bath-accessory` לא קיים בסכמה עדיין. שאלה: האם לסווג כ-type-bath-accessory או לא לכלול ב-Layer 6?
...
**מה אייל צריך לבדוק:**
- [ ] ~~גיל~~: **Phase 5b — אין חובת גיל לאביזר/מדחום** ✅
- [ ] type: האם `type-bath-accessory` נכון? (הסכמה עדיין לא כוללת אותו)
- [ ] האם מוצר זה שייך ל-Layer 6 בכלל, או EXCLUDED?
- [ ] האם יש מוצרים דומים (אלקטרוניקה לתינוק) שצריך טיפול?
...
### מוצר 12 — ✅ PASS (yaml_gap) — ⚡ type-toy: גיל לא נדרש (Phase 5b). כותרת חצויה לבדיקה.
...
**missing / review reason:** דובי צעצוע. ⚡ **Phase 5b:** type-toy → **אין חובת גיל** — PASS מוצדק. אין מגדר, אין עונה — בסדר לצעצוע. **הכותרת בשופיפיי חצויה** ("Birthday Gift for..." — חסר קצה). לבדוק.
...
- [ ] ~~גיל~~: **Phase 5b — אין חובת גיל לצעצוע** ✅ (אם יש גיל מומלץ מפורש — ניתן להוסיף)
...
**הבעיה המעודכנת (Phase 5b):**  
מתוך 31 NO_AGE_FOUND — רק ביגוד/נעליים הם בעיה אמיתית.  
ריבורן (9) + צעצועים + אביזרים → **לא בעיה, פטורים מ-CAT-B.**  
NO_AGE_FOUND אמיתי לטיפול (clothing/shoes בלבד): ~18 מוצרים.
...
**⚡ החלטה סופית (Phase 5b — אישור אייל):**

**אין חובת age לריבורן / צעצועים / אביזרים.**  
Age tag ניתן **רק** אם גיל מומלץ כתוב במפורש במוצר.

**אפשרויות שהיו בשיקול:**
- **A.** ~~בלי age tag~~ → **זו ההחלטה הנוכחית ✅**
- **B.** tag מיוחד כמו `age-not-applicable` → לא נדרש
- **C.** להשתמש בגיל מומלץ רק אם כתוב במפורש → **לעתיד**

**החלטת אייל:** ✅ **A עכשיו, C בעתיד**
