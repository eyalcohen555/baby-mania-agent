...
- **אסור תמונת מאמר בכרטיס מוצר:** כל `product-card-inline` חייב להשתמש בתמונת המוצר האמיתית מ-Shopify — **לא** תמונת גוף המאמר. זיהוי שגיאה: אותה תמונה מופיעה גם ב-`figure.article-image` וגם ב-`product-card-inline` = שגיאה
- **אסור handle מוצר מומצא:** כל `href` למוצר חייב להיות handle שקיים בחנות. אסור לנחש handle. אם לא נמצא `product_handle` ב-YAML של ה-hub — **אל תוסיף product link כלל**. אין fallback ל-`/collections/` ואין יצירת slug ידני
...
**כלל דקדוק — "תינוק בן X חודשים":**
- בשאלות FAQ, ב-JSON-LD ובגוף המאמר: תמיד "תינוק **בן** X חודשים" — זו עברית תקנית
- ✓ "כמה שעות תינוק בן 3 חודשים ישן?"
- ✗ "כמה שעות תינוק 3 חודשים ישן?" — שגיאת דקדוק בטקסט שמוצג
- **חריג:** target_keyword, כותרות H1/H2 מבוססות-keyword — "תינוק 3 חודשים" מקובל כניסוח SEO
- כלל: כל מקום שמדבר **על** תינוק ספציפי = "בן". keyword SEO = ללא "בן"
...
> ⛔ **כלל ייחודיות תמונות:**
> 1. תמונת hero חייבת להיות **שונה** מכל תמונה אחרת במאמר (גוף + כרטיסי מוצר)
> 2. כל `src=""` בתוך מאמר אחד חייב להיות **ייחודי** — אותו URL פעמיים = שגיאה
> 3. **תמונת `product-card-inline` חייבת להיות תמונת המוצר מ-Shopify** — אסור לשים תמונת גוף המאמר בכרטיס מוצר
> 4. **אסור שני כרטיסי מוצר לאותו product handle** — אם המוצר כבר הופיע, אל תחזור עליו
> 5. אל תעתיק תמונות מהפילאר לסאפורטינג
...
> ⛔ **LOCKED RULE — Handle Source + Validation (2026-03-10, enforced 2026-04-12):**
>
> **מקור יחיד מותר:** שדה `product_handle` מקובץ `shared/product-context/*.yaml` של ה-hub.
> **copy-paste בלבד** — אסור לקצר, לתרגם, לשלב, או לייצר slug מהשם המסחרי של המוצר.
>
> **בדיקת תקינות handle לפני שימוש:**
> 1. מכיל רק `a-z`, `0-9`, `-` — אם יש עברית / `™` / `%` / אותיות גדולות → **עצור, אל תשתמש**
> 2. לא נמצא בשדה `product_handle` ב-YAML → **עצור, אל תוסיף product link**
> 3. מחזיר HTTP 200 (agent09/10 יאמתו — אל תמציא אם לא ידוע)
>
