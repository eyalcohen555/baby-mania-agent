    ...
    while time.time() < deadline:
        ...
        if not saw_running:
            ...
            if status in ("done", "idle", "token_safe_stop"):
                ...
                # Bridge waiting for Telegram response — extend deadline while
                # response file is empty so we do not timeout while user is prompted.
                try:
                    _resp = (BRIDGE_DIR / "telegram-response.md").read_text(encoding="utf-8").strip()
                except Exception:
                    _resp = ""
                if not _resp:
                    deadline += POLL_INTERVAL  # pause timeout counting
            elif status == "awaiting_approval":
                # T3 manual approval — keep polling, let deadline run normally
                pass
...
def notify_conductor(event: str, plan_id: str, stage_id: str = "", detail: str = ""):
    """Write a plan-level event to bridge/conductor-notify.md for telegram_bot.py."""
    try:
        CONDUCTOR_NOTIFY_FILE.write_text(
            f"EVENT: {event}\n"
            f"plan_id: {plan_id}\n"
            f"stage_id: {stage_id}\n"
            f"detail: {detail}\n"
...
def run_plan(plan_file: str, dry_run: bool = False, resume: bool = False,
    ...
    if current_stage_id == "DONE":
        ...
        notify_conductor("PLAN_COMPLETE", plan_id, detail="PASS")
        ...
        notify_conductor("PLAN_COMPLETE", plan_id, detail=state.get("overall_verdict", "FAILED"))
