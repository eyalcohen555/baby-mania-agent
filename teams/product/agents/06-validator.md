...
## CATEGORY GATE — בצע ראשון, לפני כל בדיקה אחרת

קרא את השדה `product_template_type` מתוך `shared/product-context/{pid}.yaml`.

| ערך | נתיב |
|---|---|
| `clothing` | הפעל: Stage 2 Clothing (T01–T08) + תנאי מעבר Clothing |
| `shoes` | הפעל: Stage 2 Shoes (T01 + T02s–T04s) + תנאי מעבר Shoes |
| כל ערך אחר | הפעל: Stage 2 Clothing כברירת מחדל + WARN: "category unknown, defaulting to clothing path" |

אם `product_template_type` חסר לגמרי → **STATUS: FAIL** מיידי:
```
✗ CATEGORY GATE FAIL: product_template_type חסר מ-{pid}.yaml — לא ניתן לנתב validation
```

רשום את ה-category בראש דוח הפלט:
```
category: clothing | shoes | unknown
```

---

## Stage 2 — Thinking Layer Enforcement
...
### T02 — Schema: מבנה thinking.yaml תקין — **CLOTHING ONLY**

> דלג על T02–T08 אם `category == shoes`. עבור לבלוק **Stage 2 Shoes** למטה.
...
### T03 — Frequency Caps: מגבלות תדירות אות — **CLOTHING ONLY**
...
### T04 — Forbidden Leakage: זיהוי דליפת אשכול אסור — **CLOTHING ONLY**
...
### T05 — ER Purity: emotional_reassurance נקי מאשכולות אסורים — **CLOTHING ONLY**
...
### T06 — whats_special Source: מקור תוכן כשfallback — **CLOTHING ONLY**
...
### T07 — Care Cluster Diversity: גיוון אשכולות ב-care — **CLOTHING ONLY**
...
### T08 — Required Benefits Cards: כרטיסי חובה לא הושמטו — **CLOTHING ONLY**
...
## Stage 2 Shoes — Thinking Layer Enforcement — **SHOES ONLY**

> הפעל רק אם `category == shoes`. דלג על כל T02–T08 שמעל.

### T02s — Schema: מבנה thinking.yaml לנעליים

בדוק שהשדות הבאים קיימים ולא ריקים ב-`{pid}_thinking.yaml`:

- `product_id` (string)
- `track` — חייב להיות אחד מ: `functional` / `style` / `mixed`
- `core_angle` (string)
- `primary_parent_outcome` (string)
- `emotional_trigger` — אחד מ: `guilt` / `morning_stress` / `event_pride` / `relief` / `sizing_anxiety`
- `primary_friction` — אחד מ: `dressing_friction` / `walking_friction` / `stability_friction` / `development_friction` / `sizing_friction` / `event_friction`
- `message_budget` עם כל הסקשנים: `benefits`, `accordion_blocks`, `faq`, `sizes`
- `cluster_map` — לא ריק
- `active_forbidden_rules` — רשימה עם לפחות פריט אחד

בדיקות נוספות:
- לכל סקשן ב-`message_budget`: קיים `cluster` ו-`forbidden` (רשימה)
- **חוק cluster≠forbidden**: הערך ב-`cluster` של כל סקשן אסור שיופיע ברשימת ה-`forbidden` של אותו סקשן

כישלון בכל שדה → **FAIL**.

---

### T03s — Cluster Caps: מגבלות תדירות לנעליים

בדוק את כל קבצי הפלט: `{pid}_benefits.txt`, `{pid}_accordion.txt`, `{pid}_faq.txt`

**מילות זיהוי לפי cluster:**
- `morning_ease` — חפש: `בבוקר`, `לנעול לבד`, `יוצאים בזמן`, `שיתוף פעולה`, `ילד שמסכים`, `בלי ויכוח`
- `development_movement` — חפש: `כף הרגל`, `סוליה גמישה`, `מתפתחת`, `שרירים`, `הליכה`, `קשת`
- `stability_confidence` — חפש: `אחיזה`, `לא מחליק`, `יציב`, `נפילות`, `משטח`
- `style_emotion` — חפש: `עיצוב`, `יפה`, `שמח`, `נראה`, `מיוחד`, `בולט`
- `independence_milestone` — חפש: `לבד`, `בעצמו`, `ביטחון עצמי`, `אני מסוגל`, `עצמאות`
- `sizing_fit` — חפש: `מידה`, `למדוד`, `כמה זמן`, `גדל`, `טבלת מידות`, `סנטימטר`
- `event_occasion` — חפש: `אירוע`, `ברית`, `צילומים`, `לא מוריד`, `יום הולדת`

**חוקי Cap לנעליים:**
- `morning_ease` מופיע ב-4 מקומות ומעלה → **FAIL** (max 3)
- `development_movement` מופיע מחוץ ל-`accordion_blocks` → **FAIL**
- `sizing_fit` מופיע ב-`benefits` או ב-`hero` → **FAIL**
- `event_occasion` מופיע ביותר מ-card אחד ב-`benefits` → **FAIL**
- `development_movement` מופיע ב-3 מקומות ומעלה → **FAIL** (max 2)

---

### T04s — Forbidden Leakage לנעליים

מיפוי סקשן → קובץ:

| סקשן | קובץ |
|---|---|
| benefits | `{pid}_benefits.txt` |
| accordion_blocks | `{pid}_accordion.txt` |
| faq | `{pid}_faq.txt` |
| sizes | `{pid}_sizes.txt` (אם קיים) |

לכל סקשן:
1. קרא את `forbidden` מ-`message_budget` ב-thinking.yaml
2. לכל cluster ברשימת forbidden — בדוק אם מילות הזיהוי שלו (ראה T03s) מופיעות בטקסט הסקשן
3. מציאה → **WARN**
4. אחרי 3 WARNs ומעלה → **FAIL**

**בדיקות אוטומטיות נוספות (ללא תלות ב-thinking.yaml):**
- `sizing_fit` keywords ב-benefits → **FAIL** (תמיד אסור)
- `development_movement` keywords ב-benefits → **FAIL** (תמיד אסור)
- `development_movement` keywords ב-faq → **WARN**

---

### T05s — Benefits Cards לנעליים

קרא את `{pid}_benefits.txt`:
1. **4 או 6 כרטיסים בלבד** — FAIL אם מספר אחר
2. כל כרטיס בפורמט: emoji | כותרת | תיאור
3. אין שני כרטיסים מאותו cluster (לפי מילות זיהוי T03s) — WARN אם נמצא
4. אין `sizing_fit` או `development_movement` בכרטיסים — FAIL אם נמצא

---

### Stage 2 Summary Block (חובה בכל ריצה)

פלט בלוק זה בסיום Stage 2 (לפי category):

**אם clothing:**
```
── Stage 2 Thinking Layer (clothing) ──────────────
...
**אם shoes:**
```
── Stage 2 Thinking Layer (shoes) ──────────────────
T01 thinking.yaml present:    [PASS / FAIL]
T02s schema valid:            [PASS / FAIL] track={value}
T03s cluster caps:            [PASS / FAIL] {violations if any}
T04s forbidden leakage:       [PASS / WARN×{n} / FAIL] {details if any}
T05s benefits cards:          [PASS / WARN / FAIL] count={n}
────────────────────────────────────────────────────
Stage 2 result: [PASS / FAIL]
```
...
> בדיקות המסומנות **[CLOTHING]** חלות רק על `category == clothing`.
> בדיקות המסומנות **[SHOES]** חלות רק על `category == shoes`.
> בדיקות ללא סימון — **משותפות לכל הקטגוריות**.
...
### emotional_reassurance — **[CLOTHING]**
...
### accordion_blocks — **[SHOES]**
20s. קיימים 2-3 בלוקים ב-`{pid}_accordion.txt`
21s. כל בלוק בפורמט: כותרת | גוף | חיבור למוצר (3 חלקים)
22s. אין חזרה על תוכן שכבר ב-benefits — כל בלוק מלמד משהו שלא אמר benefits card
...
25. **[CLOTHING]** אסור להשתמש ב"סרבל" אלא אם product_type הוא סרבל במפורש
26. **[CLOTHING]** אסור להשתמש ב"100% כותנה" / "כותנה סרוגה" כשה-fabric_type ריק

### Intelligence Layer Blacklist (Step 6 — חובה כשמופעל Product Intelligence)
I01. **[CLOTHING] FAIL** אם מופיע "כותנה רכה" בכל שדה תוכן — חייב להיות שם בד ספציפי מה-context
I02. **[CLOTHING] FAIL** אם מופיע "בד רך ונעים" — חייב להחליף בניסוח ספציפי (clothing_type, print, sleeve)
I03. **FAIL** אם מופיע "איכות גבוהה" — ביטוי גנרי אסור (כל קטגוריה)
I04. **FAIL** אם מופיע "יוניסקס" — חייב לשאוב מ-gender_signal ב-Visual JSON בלבד; אם חסר → לא לאזכר גנדר (כל קטגוריה)
I05. **FAIL** אם מופיע "נוחות מושלמת" — ביטוי אסור (כל קטגוריה)
I06. **[SHOES] FAIL** אם מופיע "אורתופדי", "יפתח קשת", "ימנע שטוח רגל" — תביעת התפתחות לא מאומתת
I07. **[SHOES] FAIL** אם מופיע "ברך שרוטה", "בלי נפילות", "לא יכאב" — תמונות שליליות אסורות
