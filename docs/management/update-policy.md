...
## Commit / Push Policy — מניעת כפילות עבודה

**ברירת מחדל חדשה:** לא עושים `commit` / `push` אחרי כל קובץ קטן.

### Batch mode — מקבץ עבודה
כאשר יש כמה מסמכים / בדיקות / שלבים מאותו נושא:
1. יוצרים את כל הקבצים באותה משימה.
2. מבצעים self-review בסוף המקבץ.
3. מבצעים `commit` + `push` אחד בלבד בסוף.
4. עושים verify אחד בלבד אחרי ה-push.

### Mission mode — משימה מלאה מקצה לקצה
כאשר המשימה מוגדרת היטב:
1. יצירה / תיקון.
2. self-review.
3. עדכון תיעוד אם צריך.
4. `commit` + `push`.
5. verify.

### מתי עדיין עושים push מיידי
- סגירת milestone משמעותי.
- לפני מעבר chat / handoff.
- שינוי שחייב להפוך מיד ל-source of truth.
- מצב שבו המשך העבודה תלוי בקריאת GitHub עדכנית.

### מתי לא עושים push מיידי
- אחרי יצירת מסמך בודד מתוך סדרת מסמכים.
- אחרי audit בלבד.
- אחרי output ביניים.
- כשידוע שיש עוד 2–5 צעדים באותו scope.

**כלל עבודה:** אם יש סדרת פעולות מאותו scope — לעבוד ב-Batch mode. אם זו משימה אחת מלאה — לעבוד ב-Mission mode. לא לפצל פעולה ל-create ואז push כפקודה נפרדת, אלא אם יש סיבה קריטית.
