"""
Phase 7C Live Batch 4.
T3 approval received from Ayal. Reads phase7c-batch4-plan.json.
...
BACKUP_JSON  = "output/tags/phase7c-live-batch4-backup.json"
DRYRUN_JSON  = "output/tags/phase7c-live-batch4-dry-run.json"
DRYRUN_MD    = "output/tags/phase7c-live-batch4-dry-run.md"
ROLLBACK_MD  = "output/tags/phase7c-live-batch4-rollback-plan.md"
VERIFY_JSON  = "output/tags/phase7c-live-batch4-verify.json"
VERIFY_MD    = "output/tags/phase7c-live-batch4-verify.md"
...
def run_dry_run(plan_products, max_n, do_normalize):
    log("=" * 62)
    log("Phase 7C Live Batch 4 — DRY RUN")
    log("=" * 62)
...
def _write_dryrun_md(result, rows):
    lines = [
        "# Phase 7C Live Batch 4 — Dry Run Report",
        "",
...
def run_live(plan_products, max_n, do_normalize):
    log("=" * 62)
    log("Phase 7C Live Batch 4 — LIVE WRITE")
    log("=" * 62)
    ...
    backup = {
        "phase": "7C", "batch": 4, "timestamp": ts0,
    ...
    pass_n  = sum(1 for r in rows if r["final_verdict"] == "PASS")
    fail_n  = sum(1 for r in rows if r["final_verdict"] == "FAIL")
    verdict = "PHASE7C_LIVE_BATCH4_PASS" if all_pass and fail_n == 0 else "HOLD_PHASE7C_LIVE_BATCH4"
    ...
    if verdict != "PHASE7C_LIVE_BATCH4_PASS":
        sys.exit(1)
...
def _write_verify_md(result, rows):
    lines = [
        "# Phase 7C Live Batch 4 — Verify Report",
        "",
        f"**Timestamp:** {result['timestamp']}  ",
...
def main():
    ...
    parser.add_argument("--plan", default="output/tags/phase7c-batch4-plan.json")
