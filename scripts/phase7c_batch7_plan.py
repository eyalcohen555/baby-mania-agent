"""
Phase 7C Batch 7 — READ-ONLY Planning.
...
Explicitly excludes all product IDs already written in Batch 1/2/3/4.
...
OUT_MD   = "output/tags/phase7c-batch7-plan.md"
OUT_JSON = "output/tags/phase7c-batch7-plan.json"

HEBREW_MONTH_SINGULAR = {
    "0-3 חודש", "3-6 חודש", "6-12 חודש", "12-18 חודש", "18-24 חודש",
}

# All product IDs already written in Phase 7C Batch 1/2/3/4/5/6
ALREADY_WRITTEN_PIDS = {
    ...
    # batch6
    "9606694011193","9895864402233","9605887590713","9892620960057","9096607400249","9096606810425","9688965022009","9096606974265","10005779710265","9179167654201","9179170799929","9606691356985","9179157725497","9179133870393","9179158839609","9179164705081","9179166376249","9179148190009","9179168473401","9179170144569",
}
...
NOT_SET_TITLE_KW      = ["מגבת", "towel", "אמבטיה", "bath", "בגד ים", "swimsuit", "swimwear",
                         "כובע", "יחידת", "מארז", "מברשות", "מברשת", "מתלה", "ניקוי"]
...
def classify_product(p):
    ...
    if pid in ALREADY_WRITTEN_PIDS:
        return {"status": "already_written_batch123456", "product_id": pid, "title": title}
    ...
    reason_selected += ", no false-positive flags, no shoe title, active product, not in batch1/2/3/4/5/6"
...
def run_safety_checks(candidates):
    ...
    for c in candidates:
        ...
        if c["product_id"] in ALREADY_WRITTEN_PIDS:
            flags.append(f"{pid}: ALREADY_WRITTEN_batch123456")
...
def main():
    ...
    log("=" * 62)
    log("Phase 7C Batch 7 — READ-ONLY Planning")
    log("=" * 62)
    ...
    for p in products:
        ...
        s = r["status"]
        if s == "already_written_batch123456":
    ...
    log(f"  already_written_batch123456: {len(already_written)}")
    ...
    verdict = "READY_FOR_PHASE7C_BATCH7_T3_APPROVAL" if (
        all_pass and len(candidates) > 0
    ) else "HOLD_PHASE7C_BATCH7_NEEDS_FIX"
    ...
    return 0 if verdict == "READY_FOR_PHASE7C_BATCH7_T3_APPROVAL" else 1
...
def _write_json(ts, candidates, type_counts, all_pass, safety_flags, verdict,
    ...
    data = {
        "phase": "7C",
        "batch": 7,
        "task": "batch7_read_only_plan",
        "mode": "READ_ONLY_PLANNING",
        "timestamp": ts,
        "shopify_writes": "NONE",
        "total_active_products_fetched": total_active,
        "already_written_batch123456": already_written_count,
        ...
        "excluded": [
            "already written in Batch 1/2/3/4 (explicit PID list)",
...
def _write_md(ts, candidates, type_counts, all_pass, safety_flags, verdict,
    ...
    lines = [
        ...
        f"| Already written Batch 1/2/3/4/5/6 (excluded) | {already_written_count} |",
    ...
    for i, c in enumerate(candidates, 1):
        ...
        lines += [
            ...
            f"- not_in_batch123456_check: ✅ PASS",
            "",
            "---",
            "",
        ]
    ...
    lines += [
        ...
        f"| Not in Batch 1/2/3/4/5/6 | {'✅ PASS' if all_pass else '❌ FAIL'} |",
    ...
    if verdict == "READY_FOR_PHASE7C_BATCH7_T3_APPROVAL":
        lines += [
            f"✅ {len(candidates)} SAFE candidates selected.",
            "All safety checks PASS. No age-* tags. No type collision. No forbidden tags.",
            f"No overlap with Batch 1/2/3/4/5/6 written products. {month_norm_count} product(s) need Hebrew month normalization in live stage.",
    ...
    lines += ["", "---", "", "*Generated by scripts/phase7c_batch7_plan.py*"]
