"""
Phase 7C Live Batch 7.
T3 approval received from Ayal. Reads phase7c-batch7-plan.json.
...
BACKUP_JSON  = "output/tags/phase7c-live-batch7-backup.json"
DRYRUN_JSON  = "output/tags/phase7c-live-batch7-dry-run.json"
DRYRUN_MD    = "output/tags/phase7c-live-batch7-dry-run.md"
ROLLBACK_MD  = "output/tags/phase7c-live-batch7-rollback-plan.md"
VERIFY_JSON  = "output/tags/phase7c-live-batch7-verify.json"
VERIFY_MD    = "output/tags/phase7c-live-batch7-verify.md"
...
def select_batch(plan_products, max_n, exclude_titles=None):
    excluded_shoe = []
    excluded_title = []
    candidates = []
    for p in plan_products:
        if is_shoe_title(p["title"]):
            ...
            candidates.append(p)
    return candidates[:max_n], excluded_shoe, excluded_title


# ── DRY RUN ───────────────────────────────────────────────────────────────────
def run_dry_run(plan_products, max_n, do_normalize, exclude_titles=None):
    log("=" * 62)
    log("Phase 7C Live Batch 7 — DRY RUN")
    log("=" * 62)

    selected, excluded, excluded_t = select_batch(plan_products, max_n, exclude_titles)
...
def _write_dryrun_md(result, rows):
    lines = [
        "# Phase 7C Live Batch 7 — Dry Run Report",
        "",
...
# ── LIVE WRITE ────────────────────────────────────────────────────────────────
def run_live(plan_products, max_n, do_normalize, exclude_titles=None):
    log("=" * 62)
    log("Phase 7C Live Batch 7 — LIVE WRITE")
    log("=" * 62)
    ...
    selected, _, _t = select_batch(plan_products, max_n, exclude_titles)
    ...
    backup = {
        "phase": "7C", "batch": 7, "timestamp": ts0,
    ...
    pass_n  = sum(1 for r in rows if r["final_verdict"] == "PASS")
    fail_n  = sum(1 for r in rows if r["final_verdict"] == "FAIL")
    verdict = "PHASE7C_LIVE_BATCH7_PASS" if all_pass and fail_n == 0 else "HOLD_PHASE7C_LIVE_BATCH7"
    ...
    if verdict != "PHASE7C_LIVE_BATCH7_PASS":
        sys.exit(1)
...
def _write_verify_md(result, rows):
    lines = [
        "# Phase 7C Live Batch 7 — Verify Report",
        "",
        f"**Timestamp:** {result['timestamp']}  ",
...
def main():
    ...
    parser.add_argument("--plan", default="output/tags/phase7c-batch7-plan.json")
    ...
    if args.mode == "dry-run":
        ok = run_dry_run(plan_products, args.max_products, do_normalize, exclude_titles)
        sys.exit(0 if ok else 1)
    elif args.mode == "live":
        run_live(plan_products, args.max_products, do_normalize, exclude_titles)
