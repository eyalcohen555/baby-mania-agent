"""
Phase 7C Live Batch 9.
T3 approval received from Ayal. Reads phase7c-batch9-plan.json.
Types: type-set only (all other pools exhausted). Max 20 products.
...
--exclude-product-id: comma-sep PIDs to exclude (sweater 10011383202105 excluded by default).
"""
import argparse, json, sys, time, datetime, urllib.request, urllib.error, urllib.parse

ENV_PATH_PROJECT = r"C:\Projects\baby-mania-agent\.env"
ENV_PATH_DESKTOP = r"C:\Users\3024e\Desktop\shopify-token\.env"
...
def _fetch_oauth_token(shop, client_id, client_secret):
    """Get a fresh access token via client_credentials OAuth flow (~24h TTL)."""
...
env = load_env(ENV_PATH_PROJECT)
env.update(load_env(ENV_PATH_DESKTOP))  # desktop overrides project
SHOP          = env["SHOPIFY_SHOP_URL"].replace("https://", "").rstrip("/")
CLIENT_ID     = env["SHOPIFY_CLIENT_ID"]
CLIENT_SECRET = env["SHOPIFY_CLIENT_SECRET"]
TOKEN         = _fetch_oauth_token(SHOP, CLIENT_ID, CLIENT_SECRET)
...
BACKUP_JSON  = "output/tags/phase7c-live-batch9-backup.json"
DRYRUN_JSON  = "output/tags/phase7c-live-batch9-dry-run.json"
DRYRUN_MD    = "output/tags/phase7c-live-batch9-dry-run.md"
ROLLBACK_MD  = "output/tags/phase7c-live-batch9-rollback-plan.md"
VERIFY_JSON  = "output/tags/phase7c-live-batch9-verify.json"
VERIFY_MD    = "output/tags/phase7c-live-batch9-verify.md"
...
def select_batch(plan_products, max_n, exclude_titles=None, exclude_pids=None):
    excluded_shoe  = []
    excluded_title = []
    excluded_pid   = []
    candidates     = []
    for p in plan_products:
        ...
        elif exclude_pids and pid in exclude_pids:
            excluded_pid.append(p)
        elif exclude_titles and any(ex.lower() in p["title"].lower() for ex in exclude_titles):
            excluded_title.append(p)
        else:
            candidates.append(p)
    return candidates[:max_n], excluded_shoe, excluded_title + excluded_pid


# ── DRY RUN ───────────────────────────────────────────────────────────────────
def run_dry_run(plan_products, max_n, do_normalize, exclude_titles=None, exclude_pids=None):
    log("=" * 62)
    log("Phase 7C Live Batch 9 — DRY RUN")
    log("=" * 62)

    selected, excluded_shoe, excluded_other = select_batch(plan_products, max_n, exclude_titles, exclude_pids)
    log(f"\n[select] {len(plan_products)} in plan → {len(excluded_shoe)} excluded (shoe) → {len(excluded_other)} excluded (pid/title) → {len(selected)} selected")

    if excluded_shoe:
        log("\n[excluded — shoe title keyword]")
        for p in excluded_shoe:
            log(f"  EXCLUDE(shoe): {p['product_id']} | {p['title']}")

    if excluded_other:
        log("\n[excluded — product_id / title filter]")
        for p in excluded_other:
            log(f"  EXCLUDE(pid/title): {p['product_id']} | {p['title']}")
...
def _write_dryrun_md(result, rows):
    lines = [
        "# Phase 7C Live Batch 9 — Dry Run Report",
        "",
...
# ── LIVE WRITE ────────────────────────────────────────────────────────────────
def run_live(plan_products, max_n, do_normalize, exclude_titles=None, exclude_pids=None):
    log("=" * 62)
    log("Phase 7C Live Batch 9 — LIVE WRITE")
    log("=" * 62)
    ...
    selected, _, _t = select_batch(plan_products, max_n, exclude_titles, exclude_pids)
    ...
    backup = {
        "phase": "7C", "batch": 9, "timestamp": ts0,
    ...
    pass_n  = sum(1 for r in rows if r["final_verdict"] == "PASS")
    fail_n  = sum(1 for r in rows if r["final_verdict"] == "FAIL")
    verdict = "PHASE7C_LIVE_BATCH9_PASS" if all_pass and fail_n == 0 else "HOLD_PHASE7C_LIVE_BATCH9"
    ...
    if verdict != "PHASE7C_LIVE_BATCH9_PASS":
        sys.exit(1)
...
def _write_verify_md(result, rows):
    lines = [
        "# Phase 7C Live Batch 9 — Verify Report",
        "",
        f"**Timestamp:** {result['timestamp']}  ",
...
def main():
    ...
    parser.add_argument("--plan", default="output/tags/phase7c-batch9-plan.json")
    parser.add_argument("--max-products", type=int, default=20)
    parser.add_argument("--normalize-hebrew-months", default="true")
    parser.add_argument("--exclude-title", default="", help="Comma-sep substrings to exclude by title")
    parser.add_argument("--exclude-product-id", default="", help="Comma-sep product IDs to exclude")
    args = parser.parse_args()

    do_normalize = args.normalize_hebrew_months.lower() == "true"
    exclude_titles = [t.strip() for t in args.exclude_title.split(",") if t.strip()]
    exclude_pids   = {pid.strip() for pid in args.exclude_product_id.split(",") if pid.strip()}

    if exclude_titles:
        log(f"[exclude_titles] {exclude_titles}")
    if exclude_pids:
        log(f"[exclude_pids] {exclude_pids}")
    ...
    if args.mode == "dry-run":
        ok = run_dry_run(plan_products, args.max_products, do_normalize, exclude_titles, exclude_pids)
        sys.exit(0 if ok else 1)
    elif args.mode == "live":
        run_live(plan_products, args.max_products, do_normalize, exclude_titles, exclude_pids)
