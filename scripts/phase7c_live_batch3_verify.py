"""
Phase 7C Live Batch 3 — Post-run verify.
...
def main():
    ...
    log("=" * 62)
    log("Phase 7C Live Batch 3 — Post-run Verify (READ-ONLY)")
    log("=" * 62)
    ...
    if live_verify["verdict"] != "PHASE7C_LIVE_BATCH3_PASS":
    ...
    for bp in backup["products"]:
        ...
        if ok:
            log(f"  ✅ {pid} — {cur_title[:45]} | {len(cur_tags)} tags")
        else:
            all_pass = False
            reasons = []
