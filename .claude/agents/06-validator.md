...
19a. **FAIL** אם השאלה הראשונה אינה "הסיפור שלנו" — שאלת trust חובה קבועה, חייבת להופיע ראשונה
19a-i. **FAIL** אם תשובת שאלת "הסיפור שלנו" אינה מכילה בדיוק את כל אלה:
     - "בייבי מניה הוא מותג ישראלי"
     - "מציעים בגדים איכותיים"
     - "שירות לקוחות אישי"
     - "משלוחים לכל הארץ"
     תשובה מאושרת: בייבי מניה הוא מותג ישראלי מוביל לביגוד תינוקות פרימיום. אנחנו מציעים בגדים איכותיים, נוחים ואופנתיים לתינוקות וילדים, עם שירות לקוחות אישי וחוויית קנייה מהנה. ומשלוחים לכל הארץ
     כל שינוי ניסוח, קיצור, או החלפה → **FAIL** + חסימת publish.
19a-ii. **FAIL** אם קיים יותר מ-Q אחד שמכיל מי אנחנו או למה לבחור בנו או מה מיוחד בנו — מופע אחד בלבד מותר.
19a-iii. **FAIL** אם מופיע הביטוי "מי אנחנו" בכל מקום ב-FAQ (שאלה, תשובה, או כל שדה אחר) — הניסוח היחיד המותר הוא "הסיפור שלנו"; "מי אנחנו" אסור לחלוטין
19b. **FAIL** אם השאלה האחרונה אינה "כמה זמן לוקח המשלוח?" — שאלת trust חובה קבועה, חייבת להופיע אחרונה
19c. בדיקת נוכחות: חפש "הסיפור שלנו" ו"כמה זמן לוקח המשלוח" בתוכן ה-FAQ. אם אחד מהם חסר לגמרי → **FAIL**
19d. **FAIL** אם תשובת שאלת המשלוח אינה מכילה בדיוק את כל אלה:
     - "5-9 ימי עסקים" (משלוח עד הבית)
     - "6-11 ימי עסקים" (נקודת איסוף)
     - "1-3 ימי עסקים" (שליחה)
     - "שישי, שבת וחגים"
     - "מספר מעקב"
     כל שינוי במספרים, מחיקת שורה, או קיצור - **FAIL** + חסימת publish.
19e. **FAIL** אם קיים יותר מ-Q אחד שמכיל "משלוח" - שאלה אחת בלבד מותרת.
...
22a. **FAIL** אם `hand_wash` מופיע עם card_title שאינה בדיוק `כביסה עדינה ביד` — ניסוחים אסורים: "כביסה ביד", "כביסת יד", "כביסה עדינה בלבד", כל וריאציה אחרת
22b. **FAIL** אם `hand_wash` הוא הכרטיס הראשון ב-care_instructions ו-fabric_type ריק או אינו מכיל בד עדין (צמר/קשמיר/משי/לייקרה/אנגורה/שיפון). hand_wash ככרטיס ראשון מותר רק כשמוצדק על ידי fabric_type.
...
18. אם fabric_type ריק — icon_type הראשון בסקשן care אסור להיות hand_wash; השתמש ב-wash_60 כברירת מחדל
...
F02. fabric_empty=true → care_instructions אסור שישתמש ב-hand_wash ככרטיס ראשון; wash_60 הוא הכרטיס הראשון הנדרש.
